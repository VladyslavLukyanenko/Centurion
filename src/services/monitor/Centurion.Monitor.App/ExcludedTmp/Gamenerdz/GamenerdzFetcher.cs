...
{
  public class GamenerdzFetcher : IProductStatusFetcher
  {
    ...
    public GamenerdzFetcher(HttpClient httpClient, Uri productUrl)
    {
    ...
    public async ValueTask<Result<StatusFetchResult>> FetchAsync(CancellationToken ct)
    {
      ...
      return await StatusFetchResult.ProcessResult(request, _httpClient, ct, result =>
      {
        ...
        var available = AvailabilityRegex.Match(content).Groups[1].Value != "oos";
