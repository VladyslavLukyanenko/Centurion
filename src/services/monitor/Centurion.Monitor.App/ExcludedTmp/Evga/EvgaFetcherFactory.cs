...
namespace Centurion.Monitor.App.Sites.Evga
{
  [Monitor("evga")]
  public class EvgaFetcherFactory : ProductStatusFetcherFactoryBase
  {
    ...
    public EvgaFetcherFactory(IServiceProvider serviceProvider)
    ...
    {
      return new EvgaFetcher(clientFactory.CreateHttpClient(), new Uri(target.Input));
    }
  }
}