...
// ALPS extension https://datatracker.ietf.org/doc/html/draft-vvv-tls-alps
type ALPSExtension struct {
	raw                []byte
	SupportedProtocols []string
}

func (e *ALPSExtension) marshal() []byte {
	if e.raw != nil {
		return e.raw
	}

	builder := newByteBuilder()
	builder.addU16(extensionApplicationSettings)
	payloadBuilder := builder.addU16LengthPrefixed()
	extContentBuilder := newByteBuilder()
	if len(e.SupportedProtocols) > 0 {
		protosBuilder := newByteBuilder()

		for ix := range e.SupportedProtocols {
			proto := e.SupportedProtocols[ix]
			protosBuilder.addU8LengthPrefixed().addBytes([]byte(proto))
		}

		extContentBuilder.addU16LengthPrefixed().addBytes(protosBuilder.finish())
	}

	payloadBuilder.addBytes(extContentBuilder.finish())

	e.raw = builder.finish()
	return e.raw
}
...
type clientEncryptedExtensionsMsg struct {
	raw                 []byte
	applicationSettings *ALPSExtension
	customExtension     []byte
}

func (m *clientEncryptedExtensionsMsg) writeToUConn(conn *UConn) error {
	return nil
}

func (m *clientEncryptedExtensionsMsg) Len() int {
	return len(m.marshal())
}

func (m *clientEncryptedExtensionsMsg) Read(p []byte) (n int, err error) {
	bytes := m.marshal()
	copy(p, bytes)
...
func (m *clientEncryptedExtensionsMsg) marshal() (x []byte) {
	...
	if m.applicationSettings != nil {
		extensions.addBytes(m.applicationSettings.marshal())
	}
...
func (m *clientEncryptedExtensionsMsg) unmarshal(data []byte) bool {
	...
	//}
	//return true
	panic("clientEncryptedExtensionsMsg unmarshal Not implemented")
}
