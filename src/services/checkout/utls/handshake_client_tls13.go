...
func (hs *clientHandshakeStateTLS13) handshake() error {
	...
	if err := hs.sendClientEncryptedMessage(); err != nil {
		return err
	}
...
func (hs *clientHandshakeStateTLS13) readServerParameters() error {
	...
	c.applicationSettings = encryptedExtensions.applicationSettings
...
func (hs *clientHandshakeStateTLS13) sendClientEncryptedMessage() error {
	if hs.c.applicationSettings == nil {
		return nil
	}

	ext := new(clientEncryptedExtensionsMsg)
	settings := new(ALPSExtension)
	settings.SupportedProtocols = make([]string, 0, 0)
	ext.applicationSettings = settings

	hs.transcript.Write(ext.marshal())

	if _, err := hs.c.writeRecord(recordTypeHandshake, ext.marshal()); err != nil {
		return err
	}
