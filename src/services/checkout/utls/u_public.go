...
func (hs *clientHandshakeStateTLS13) toPublic13() *ClientHandshakeState {
	if hs == nil {
		return nil
	} else {
		tls13State := TLS13OnlyState{
			EcdheParams:   hs.ecdheParams,
			EarlySecret:   hs.earlySecret,
			BinderKey:     hs.binderKey,
			CertReq:       hs.certReq.toPublic(),
			UsingPSK:      hs.usingPSK,
			SentDummyCCS:  hs.sentDummyCCS,
			Suite:         hs.suite.toPublic(),
			TrafficSecret: hs.trafficSecret,
			Transcript:    hs.transcript,
		}
		return &ClientHandshakeState{
			C:           hs.c,
			ServerHello: hs.serverHello.getPublicPtr(),
			Hello:       hs.hello.getPublicPtr(),
